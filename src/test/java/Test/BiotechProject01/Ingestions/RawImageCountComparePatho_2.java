...
public class RawImageCountComparePatho_2 extends DB_Config_DW {
	...
	public void CountCompare() throws InterruptedException, IOException, SQLException	{
			...
			try{
				for (ReportFilters objFilter : objModel.lstFilters) {
					...
					try{
						...
						HttpGet postRequest1 = new HttpGet(Constants.api_uploadImage);
						...
						Response response2 = request_fileupload.post(Constants.api_uploadImage);
		...
		try{
			...
			for (int x = 0;x<=100;x++) {
				...
				String query1 = "Select count(status) as count from Patho_2_OUTPUT where Sample_ID like '%"+date0+"' and Sample_ID like '"+dateYYYYMMDD+"%'";
			//	String query1 = "Select count(status) as count from Patho_2_OUTPUT where Sample_ID like '20211201%' and Sample_ID like '%1503'";
				...
				while (rs1.next()) {
					...
					if (rs1.getString("count").equals(totalImages)) {
						...
						while (i<=Integer.parseInt(totalImages)) {
							...
							String query2 = "Select w2_cell_count, lane_noise_ratio_percent, count_outcome from Patho_2_OUTPUT where Sample_ID = '"+dateYYYYMMDD+"_Salm_"+i+"_"+date0+"'";
						//	String query2 = "Select w2_cell_count, lane_noise_ratio_percent, count_outcome from Patho_2_OUTPUT where Sample_ID = '"+Test_Variables.dateYYYYMMDD+"_Salm_"+i+"_1503'";
...
//								String path = System.getProperty("user.dir")+"\\CountComparison\\CountComparison"+totalImages+".xlsx";
								String path = System.getProperty("user.dir")+"\\src\\test\\resources\\CountComparison\\CountComparison"+totalImages+".xlsx";
